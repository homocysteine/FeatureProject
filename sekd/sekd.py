...
class SEKD(object):
    ...
    def __init__(self, weights_path='../model/sekd.pth', confidence_threshold = 0.55,
                 nms_radius = 4, max_keypoints = 500, cuda = True,
                 multi_scale = True, sub_pixel_location = False):
    ...
    def detectAndCompute(self, img, mask = None):
        ...
        # print('SEKD1 keypoints length ',len(keypoints))
        keypoints = keypoints
        scores = keypoints[2,:]
        keypoints = keypoints[:2].T
        descriptors = descriptors.T
        inds = np.argsort(scores)[::-1] #A list save the index from the highest score to lowest score
        nfeatures = min(len(scores),self.max_keypoints)
        inds = inds[:nfeatures] #select 500 index
        scores = scores[inds] # use index to find the corresponding score
        descriptors = descriptors[inds, :]
        keypoints = keypoints[inds,:]

        # keypoints [Tensor(nx2)] 2 refers to x,y
        # scores (Tensor(n))
        # descriptor [Tensor(128xn)]
    ...
    def detectAndComputeOneScale(self, img, mask = None):
        ...
        with torch.no_grad():
            ...
            # to work with superglue,raise the dim of descriptor to 256
            # descriptors = torch.nn.functional.normalize(descriptors).reshape([256, -1])

            descriptors = descriptors.detach().cpu().numpy()

        return keypoints, descriptors

    def detectForEval(self,img,mask=None):
        if len(img.shape) == 3:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        height, width = img.shape
        ...
        descriptors = descriptors.T
        inds = np.argsort(scores)[::-1] #A list save the index from the highest score to lowest score
        inds = inds[:self.max_keypoints] #select 500 index
        scores = scores[inds] # use index to find the corresponding score
        keypoints = keypoints[inds, :].T # eliminate the keypoint with low score
        # keypoints.append(scores)
        # np.append(keypoints,scores)
        scores = scores[np.newaxis,:]
        keypoints = np.concatenate((keypoints,scores),axis=0)
