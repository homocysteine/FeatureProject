...
methods = {
    ...
    'superpoint':{'name':'SuperPoint','color':'red','linestyle':'-','marker':'v'},
...
dataset_path = '/home/yushichen/PycharmProjects/pythonProject/data/hpatches-dataset/hpatches-sequences-resize'
features_path = '/media/yushichen/DATA/Datasets/MyPlatformFeature/feature'
...
def benchmark_features(read_feats, method=None):
    ...
    for seq_idx, seq_name in tqdm(enumerate(seq_names), total=len(seq_names)):
        ...
        for im_idx in range(2, 7):
            ...
            if not os.path.isdir(os.path.join('/media/yushichen/DATA/Datasets/MyPlatformFeature/hpatches_stitched', seq_name)):
                os.makedirs(os.path.join('/media/yushichen/DATA/Datasets/MyPlatformFeature/hpatches_stitched', seq_name))
            cv2.imwrite(os.path.join('/media/yushichen/DATA/Datasets/MyPlatformFeature/hpatches_stitched', seq_name, "matched_1_{0}_{1}.jpg".format(im_idx, method)), img_match)
            # Draw stitch result.
            ...
            cv2.imwrite(os.path.join('/media/yushichen/DATA/Datasets/MyPlatformFeature/hpatches_stitched', seq_name, "stitched_1_{0}_{1}.jpg".format(im_idx, method)), img_stitched)
#            break
#        break
...
cache_dir = '/home/yushichen/PycharmProjects/pythonProject/data/cache_homo'
...
for method in methods:
    ...
    if os.path.exists(output_file):
        print('Loading precomputed errors...')  #ad the file already have
        errors[method] = np.load(output_file, allow_pickle=True)
    else:
        print('Computing errors...') # computing right now
...
plt.title('Overall', fontname='DejaVu Sans')
plt.xlim(plt_lim)
plt.xticks(plt_rng, fontname='DejaVu Sans')
plt.ylabel('Homography accuracy', fontname='DejaVu Sans')
plt.ylim([0, 1])
plt.yticks([x/10 for x in range(11)], fontname='DejaVu Sans')
plt.grid(linestyle=':')
...
for method in methods:
    ...
    plt.plot(plt_rng, [i_err[thr] / (n_i * 5)
            for thr in plt_rng], color=color, ls=ls, marker=marker, linewidth=2, label=name)
plt.title('Illumination', fontname='DejaVu Sans')
plt.xlabel('Threshold [px]', fontname='DejaVu Sans')
plt.xlim(plt_lim)
plt.xticks(plt_rng, fontname='DejaVu Sans')
plt.ylim([0, 1])
plt.yticks([x/10 for x in range(11)], fontname='DejaVu Sans')
...
for method in methods:
    ...
    plt.plot(plt_rng, [v_err[thr] / (n_v * 5)
            for thr in plt_rng], color=color, ls=ls, marker=marker, linewidth=2, label=name)
plt.title('Viewpoint', fontname='DejaVu Sans')
plt.xlim(plt_lim)
plt.xticks(plt_rng, fontname='DejaVu Sans')
plt.ylim([0, 1])
plt.yticks([x/10 for x in range(11)], fontname='DejaVu Sans')
...
if top_k is None:
    plt.savefig('/home/yushichen/PycharmProjects/pythonProject/data/figs/hseq-homo.pdf', bbox_inches='tight', dpi=300)
else:
    plt.savefig('/home/yushichen/PycharmProjects/pythonProject/data/figs/hseq-top-homo.pdf', bbox_inches='tight', dpi=300)
